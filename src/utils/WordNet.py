...
class WordNet():
	...
	def ReadWordNet(self, file_name_l, verbal=False, print_every = 3000000,min_freq_cutoff=3):
		...
		for i,file_name in enumerate(file_name_l):
			fin = open(self.DATA_DIR + file_name)
	...
	def ReadNodeTypeNCBI(self,default_score=100, file_name = 'NLP_Dictionary/description.txt',verbal=False, print_every = 100000):
		...
		for ct,line in enumerate(fin):
			...
			text = line.lower().translate(str.maketrans('','',',:()=%>/[]'))
	...
	def ReadNodeTypeLiterome(self, default_score=100, file_name = 'NLP_Dictionary/pubmed.network'):
	...
	def ReadNodeTypePPI(self, default_score=100, file_name = 'network/human/InBio-Map_Symbol.sif'):
	...
	def ReadNodeTypeAutoPhrase(self, file_name = 'NLP_Dictionary/AutoPhrase_multi-words.txt.only_GO_based',sc_cutoff = 0.8):
		fin = open(self.DATA_DIR + file_name)
		for line in fin:
			w = line.lower().strip().split('\t')
			sc = float(w[0])
	...
	def ReadNodeTypePercha(self, file_name = 'NLP_Dictionary/all_entity.txt'):
		fin = open(self.DATA_DIR + file_name)
		for line in fin:
			...
			if w[0].replace('_',' ') in self.word_type:
				...
				self.word_type[w[0].replace('_',' ')] ='Percha_'+w[1]
		fin.close()
	def ReadNodeTypeSider(self, default_score=100, file_drug2adr = 'side_effect/bio-decagon-mono.csv'):
		hpo_d2d = collections.defaultdict(dict)
		fin = open(self.DATA_DIR + file_drug2adr)
		for line in fin:
			w = line.strip().lower().split(',')
			self.word_type[w[2]] = 'sider_disease'
		fin.close()

	def ReadNodeTypeHPO(self, default_score=100, file_hpo2gene = 'NLP_Dictionary/hpo_gene.txt', file_phenotype = 'NLP_Dictionary/phenotype_annotation.tab',file_obo = 'NLP_Dictionary/hp_obo_parsed.tsv'):
		hpo_d2d = collections.defaultdict(dict)
		fin = open(self.DATA_DIR +file_phenotype)
		for line in fin:
			w = line.strip().lower().split('\t')
		...
		fin = open(self.DATA_DIR +file_hpo2gene)
		for line in fin:
			w = line.strip().lower().split('\t')
			d = w[1]
			g = w[3]
		...
		fin = open(self.DATA_DIR +file_obo)
		for line in fin:
			w = line.strip().lower().split('\t')
	...
	def ReadNodeTypeTissue(self, file_name = 'NLP_Dictionary/tissue.txt'):
		fin = open(self.DATA_DIR + file_name)
		for line in fin:
			w =line.strip().lower()
	...
	def ReadNodeTypeMesh(self,file_name = 'NLP_Dictionary/2017MeshTree.txt'):
	...
	def ReadNodeTypeGO(self,file_name = 'NLP_Dictionary/go_term.txt'):
		fin = open(self.DATA_DIR + file_name)
		for line in fin:
			w  =line.strip().lower()
	...
	def ReadNodeTypeCCLE(self,file_name = 'NLP_Dictionary/top_genes_exp_hgnc.txt'):
		fin = open(self.DATA_DIR + file_name)
		for line in fin:
			w = line.lower().strip().split('\t')
	...
	def ReadNodeTypeMonarchDiseaseGene(self,file_name = 'NLP_Dictionary/gene_disease_formatted.txt'):
		fin = open(self.DATA_DIR + file_name)
	...
	def ReadNodeTypeCtrpDrugGene(self,file_name = 'NLP_Dictionary/drug_target_formatted.txt'):
		fin = open(self.DATA_DIR + file_name)
		d2g = collections.defaultdict(dict)
		background_gene = set()
		for line in fin:
	...
	def ReadNodeTypeCancerGene(self,file_name = 'NLP_Dictionary/cancer_gene.txt'):
		fin = open(self.DATA_DIR + file_name)
		d2g = collections.defaultdict(dict)
		background_gene = set()
		for line in fin:
	...
	def ReadEdgeTypePerchar(self,kg_thres = 5000, default_score = 100,file_name = 'NLP_Dictionary/all_relation.txt'):
	...
	def ReadEdgeTypeGO(self,default_score = 100,file_name = 'NLP_Dictionary/GO_term.network'):
	...
	def ReadEdgeTypeGO2Gene(self,default_score = 100,file_name = 'NLP_Dictionary/go2gene_set_complete.txt'):
	...
	def ReadWordType(self,default_score = 100000,use_auto_phrase=False):
		...
		self.ReadNodeTypeCtrpDrugGene(file_name = 'drug/gdsc/drug_target_mapped.txt')
		self.ReadNodeTypeCtrpDrugGene(file_name = 'drug/ctrp/drug_target_formatted.txt')
		self.ReadNodeTypeGO()
