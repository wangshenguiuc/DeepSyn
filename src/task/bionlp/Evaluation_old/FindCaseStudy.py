...
PathRes_obj = PathwayDrugResponse(dataset = 'ccle')

for topk in [10]:
	for max_layer in [4,5]:
		for edge_wt_thres in [0.02,0.03,0.01]:
			for ci,ss in enumerate(['axitinib']):
				...
				if len(exist_pos_gene) == 0:
					continue
				result_dir = '/oak/stanford/groups/rbaltman/swang91/Sheng_repo/result/network_flow_case_study/'+str(max_layer)+'_'+str(edge_wt_thres)+'/'
				...
				for i in range(topk):
					...
					if i>topk and g not in pos_gene:
						...
						continue
					if g not in ['mc4r','rad1','fadd']:
						continue
					output_file = result_dir + s+'_'+g+'_drug_boxplot'
					pos_v,neg_v,pv = PathRes_obj.CheckPvalue(s,[g])
					print g,pv,np.shape(pos_v),np.shape(neg_v)
					plot_subnet_boxplot(output_file,pos_v,neg_v,title=g)
					...
					sent_file = result_dir + s+'_'+g + '.txt'
					print ss,g
					write_graph_edges(edge_list,sent_file)
